...
import generalDiscount from '../../manager/discount/generalDiscount';
...
class EventDetails extends Component {
  ...
  goBack = () => {
    ...
    try {
      let lastLocation = window.previousLocation || {};
      if (lastLocation.pathname == '/member/joinEventPayment') {
        let redirectLocationObj = window.redirectLocation || {};
        let redirectLocationPath = redirectLocationObj.pathname || '';
        this.navigateByUrlName(redirectLocationPath);
      } else {
  ...
  componentWillReceiveProps(res) {
    ...
    if (res.events && res.events.memberEventDetailsObjectData) {
      let numberOfLikes = res.events.memberEventDetailsObjectData.likesCount;
  ...
  initializePage() {
    ...
    let sendObject = {};
    let callerMemberUserId = '';

    try {
      callerMemberUserId = this.props.userInfo.id;
    } catch (err) {
      console.log(err);
    }
      ...
      this.props
        ...
        .then(res => {
          ...
          if (res.payload && res.payload.data) {
            if (res.payload.data.status === 404)
            ...
            else if (res.payload.data.resourceData) {
              if (!res.payload.data.resourceData.published) {
                _this.navigateByUrlName('/error');
              }
              let lat = '';
              let lon = '';
              ...
              if (res.payload.data.resourceData.address) {
                lat = parseFloat(
                  res.payload.data.resourceData.address.latitude
                );
                lon = parseFloat(
                  res.payload.data.resourceData.address.longitude
                );
                ven = res.payload.data.resourceData.venue;
              }

              if (res.payload.data.resourceData.alreadySubscribed) {
                this.setState({
                  buttonFlag: true
                });
              }

              if (res.payload.data.resourceData.endTimestamp) {
                let todayDate = new Date();
                let endDate = res.payload.data.resourceData.endTimestamp;
                let dateEndDate = new Date(endDate);
                dateEndDate.setHours(dateEndDate.getHours() + 23);
                dateEndDate.setMinutes(dateEndDate.getMinutes() + 59);
                dateEndDate.setSeconds(dateEndDate.getSeconds() + 59);
              ...
              this.setState(
                {
                  eventDetailsObject: res.payload.data.resourceData || {},
                  eventLikes: res.payload.data.resourceData.likesCount,
                  eventLati: lat,
                  eventLong: lon,
                  eventvenue: ven,
                  eventTimeZone: res.payload.data.resourceData.timeZone || '',
                  companyName: comp,
                  eventId
                },
                () => {
  ...
  goToJoinEvent = () => {
    ...
    let eventBasePrice = this.state.eventDetailsObject.fees || '';
    let eventName = this.state.eventDetailsObject.eventName || '';
    let eventCompany = '';

    if (
      this.state.eventDetailsObject &&
      this.state.eventDetailsObject.userResponse
    )
      eventCompany =
        this.state.eventDetailsObject.userResponse.organization || '';
    ...
    this.props.actionSetBuyEventDetails(buyEventDetails);
    this.props.actionSetJoinEventMembers([]);
    this.navigateByUrlName('joinEvent?eventId=' + this.state.currentEventId);
  };
                        ...
                        <div className="back-shadow">
                          <div className="contentBox">
                            <div className="imgBox">
                              {/* <img
                              src={this.state.eventDetailsObject.bannerImageURL}
                              onError={e => {
                                e.target.src = imgEventDefault;
                              }}
                            /> */}
                              <Img
                                src={
                            ...
                            <div className="evDescription">
                              ...
                              the 1500s, */}
                                {this.state.eventDetailsObject.description}
                              </p>
                              {/* <p>
                              Lorem Ipsum is simply dummy text of the printing and
                              typesetting industry. Lorem Ipsum has been the
                  ...
                  <div className="col-md-4">
                    <div className="j-i-wrapper joinEventPc">
                      ...
                      >
                        ...
                        this.state.buttonDateFlag === true ? (
                          ''
                        ) : (
                          <span
                            className="btn btn-rounded btn-theme"
                    ...
                    <div className="eventVenueWrapper">
                      ...
                      <div className="mapcontainer">
                        <p>Map:</p>
                        <div id="mapBox" className="mapSec">
  ...
  showSummaryDetails = agenda => {
    ...
    this.navigateByUrlName('/member/summaryDetails');
  };
  ...
  createLike = () => {
    ...
    this.props.actionSetLikeByEventId(newLikeData).then(res => {
      if (res.payload && res.payload.data && res.payload.data.status == 200) {
        ...
        this.props.actionMemberEventDetailsObjectData(sendObject);
      }
    });
  };
